...
- `ExtentHeap::free` (owner-local) and `ExtentHeap::complete_remote_free` (remote-completed) must validate through their own path but retire through the same private `retire` method; do not re-duplicate unpublish/remove/cache-insert logic between them.
