...
- `PageMap` owns the L1 mmap in `l1_mapping: Mutex<Option<Mapping>>` and publishes it through `l1: AtomicPtr<L1Table>` for lock-free `get`. Each `L1Entry` publishes its L2 through `AtomicPtr` and keeps `L2Mapping` (L2 `Mapping` + occupancy) mutated only while `l1_mapping` is held. Do not reintroduce empty `Mutex<()>`, `MaybeUninit` mapping cells, or unused lock-proof parameters on table methods.
