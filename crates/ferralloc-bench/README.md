# Ferralloc Benchmarks

This crate contains Ferralloc's allocator iteration benchsuite.
...
```sh
cargo bench -p ferralloc-bench
```

Run one benchmark target:

```sh
cargo bench -p ferralloc-bench --bench explicit
cargo bench -p ferralloc-bench --bench threaded
cargo bench -p ferralloc-bench --bench global_ferralloc
```

## Benchmark Families

- `explicit`: calls each allocator through `GlobalAlloc` directly and compares Ferralloc, System, mimalloc, jemalloc, and snmalloc in one Criterion report.
...
```sh
cargo run -p ferralloc-bench --bin rss
```
